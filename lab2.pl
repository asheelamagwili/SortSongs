...
sub mcw{
    ...
    # Stores the hash of the given word
    %newHash = %{$counts{$word}};
    $highest = 0;

    # Find all available 'next' words for $word & sort
    foreach my $next(sort keys %newHash)
    {
        # Find next word with highest count & make sure it doesnt already exist
        #if($word_history == 0)
        if($highest < $newHash{$next}  && $word_history{$next} != 1)
        {
            # Make it highest
            $highest = $newHash{$next};
            # Update it as best word
            $best_word = $next;
        }
